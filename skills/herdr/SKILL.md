...
Default to a sibling pane in the current tab and the current working directory. Narrow exception: automatic Pi-Herdr-Subagents write-agent isolation uses a dedicated Git worktree and branch even without an explicit topology request; interactive runs use Herdr-managed worktrees, while configured headless Pi/Claude runs create no pane. Read-only interactive Pi runs retain the sibling-pane default and pack active agent panes for space: the first two open right, the third is created down from the second and swapped above it, then fresh right columns alternate with one stacked pane (maximum two agent panes per later column). Only the orchestrator merges or rebases a retained branch.
...
For automatic write-agent isolation, Herdr 0.7.5 creates the checkout and root pane in one operation; use the returned identities, never infer focus or IDs:

```bash
herdr worktree create --cwd <cwd> --branch <unique-branch> --base <sha> --label <label> --no-focus --json
herdr worktree open --path <retained-checkout> --label <label> --no-focus --json
```

Parse `.result.root_pane.pane_id`, `.result.workspace.workspace_id`, `.result.workspace.worktree.checkout_path`, and `.result.worktree.branch/path/open_workspace_id`. Keep the checkout and branch after the agent finishes. The agent must modify only that checkout, run tests, commit, and report its branch/path/commit/changed files/verification; it must not merge, rebase, or remove it. The orchestrator reviews and merges, then may remove only a clean checkout without `--force`; dirty checkouts are retained.
...
`agent start` returns only after Herdr detects the expected agent in the same pane and considers it ready for interactive input. It defaults to a 30-second startup timeout. A newly created pane may still be starting its shell even though the creation command has returned. If `agent start` returns `agent_pane_busy`, inspect `herdr pane process-info --pane <id>` and `herdr pane read <id>` first. Retry only when the pane is clearly reaching an interactive shell prompt, and use a short bounded retry rather than an unbounded loop.
...
`agent prompt` atomically submits text and encoded Enter while honoring the pane's live bracketed-paste mode. For a ready agent, `agent prompt <target> <text> --wait` is the normal way to submit one task and wait. It waits for the first settled `idle`, `done`, or `blocked` state. Do not repeat those defaults with `--until`.

A `blocked` result is not task completion. It means the agent needs approval or an answer. Inspect the agent, let the user resolve the interaction, then wait specifically for completion while excluding `blocked` and `working`:

```bash
herdr agent wait reviewer --until idle --until done --timeout 120000
```

Standalone `agent wait` checks the current state immediately. It returns at once when that state matches any requested `--until`. Therefore, do not include `working` when the goal is completion, and do not use the default settled states after a `blocked` result because `blocked` would match immediately.

A prompt sent from a non-working state must produce an observed lifecycle change within five seconds. Otherwise Herdr returns `agent_prompt_stalled` instead of waiting indefinitely. This wait tracks lifecycle state, not an individual turn. If the agent is already working, completion of the active turn may satisfy it rather than the newly submitted prompt. For strict sequencing, wait for the current work to settle before submitting another prompt.

Use `--until` for state-specific workflows, such as waiting for an already-running agent to request input:
